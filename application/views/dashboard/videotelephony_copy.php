								...
								<div class="table-responsive">
									<table id="multi-filter-select" class="display table table-striped table-hover" >
										...
										<tbody>
											...
											<tr>
												<td>Please click the link to join <a target="_blank" href="<?= $row->message?>"><?= $row->message?></a></td>
												<td>
												<?php if($_SESSION['id_user_role'] == 4) {?>
													<div class="btn-group">
														<button type="button" class="btn btn-secondary" onclick="getEdit('<?= $row->id ?>')"><i class="fa fa-edit"></i></button>
														<button type="button" class="btn btn-danger" onclick="getDelete('<?= $row->id ?>','<?= base_url()?>videotelephony/delete')"><i class="fa fa-trash"></i></button>
													</div>
												<?php }?>
												<?php if($_SESSION['id_user_role'] == 3) {?>
													<div class="btn-group">
														<button type="button" class="btn btn-success" title="Accept Invitation" onclick="getEdit('<?= $row->id ?>')"><i class="fa fa-plus"></i></button>
														<button type="button" class="btn btn-danger" title="Reject Invitation" onclick="getDelete('<?= $row ->id ?>','<?= base_url()?>videotelephony/delete')"><i class="fa fa-trash"></i></button>
													</div>
												<?php }?>
												</td>
											</tr>
